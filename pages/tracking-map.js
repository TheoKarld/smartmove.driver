...
const generatePathCoordinates = () => {
  ...
  // Much larger base movement radius (approximately 100-200km)
  const baseRadius = 1.5;

  // Create figure-8 pattern with larger radius
  const lat =
    40.7128 +
  ...
  const lng =
    -74.006 +
    (baseRadius * 2 * Math.cos(time * 0.15) + // Wider east-west movement
...
export default function TrackingMap() {
  ...
  const calculateMapPosition = (coord) => {
    const scale = 20; // Adjusted scale for wider movement visibility
    return {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setPathHistory((prev) => [...prev.slice(-50), newLocation]); // Increased trail length
