...
export default function Verified(props) {
  const [isChecked, setIsChecked] = useState(false),
    ...
    { auth } = route.query;
  clg(auth);
  //`https:smartmove.vercel.app/verify?auth=${user.id}`
  useEffect(() => {
    // Animate check icon
    ...
    var access = window.localStorage.getItem("smartAccess");
    if (access) window.localStorage.removeItem("smartAccess");
  }, []);
